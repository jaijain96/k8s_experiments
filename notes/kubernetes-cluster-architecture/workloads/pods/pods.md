...
manage in Kubernetes. <mark>A `Pod` is a group of one or more `Container`s,
with shared storage and network resources, and a specification for how to run
the `Container`s. A `Pod`'s contents are always co-located and co-scheduled,
...
on the same logical host.</mark>

<mark>DOUBT: can a single `Pod` span multiple `Node`s? few search references arnd this:</mark>

1. <mark><https://www.google.com/search?q=can+one+single+pod+be+running+on+2+machines&oq=can+one+single+pod+be+running+on+2+machines&gs_lcrp=EgZjaHJvbWUyBggAEEUYOTIGCAEQLhhA0gEHMjc0ajBqMagCALACAA&sourceid=chrome&ie=UTF-8></mark>
2. <mark><https://serverfault.com/questions/809996/can-a-kubernetes-pod-span-nodes?newreg=213e7ecf8cfd4813a6c36fefa7fe0c24></mark>

The above doubt probably doesn't make sense when it's clearly stated above that:
> A `Pod`'s contents are always co-located and co-scheduled, and run in a
> shared context. A `Pod` models an application-specific "logical host": it
> contains one or more application `Container`s which are relatively tightly
> coupled. In non-cloud contexts, applications executed on the same physical
> or virtual machine are analogous to cloud applications executed on the same
> logical host.

so a `Pod` should always be on a single `Node`.
...
that `Pod`s can run there. <mark>The shared context of a `Pod` is a set of
Linux `namespace`s, `cgroup`s, and potentially other facets of isolation - the
same things that isolate a `Container`. Within a `Pod`'s context, the
individual applications may have further sub-isolations applied.</mark>

A `Pod` is similar to a set of `Container`s with shared `Namespace`s and
...
1. `Pod`s that run a single `Container`: The "one-container-per-pod" model is
   the most common Kubernetes use case; we can think of a `Pod` as a wrapper
   around a single `Container`; Kubernetes manages `Pod`s rather than managing
...
<mark>Usually we don't need to create `Pod`s directly, even singleton `Pod`s.
...
remains on that `Node` until either the `Pod` finishes execution, or the
`Pod` object is deleted, or the `Pod` is evicted for lack of resources, or
the `Node` fails. Restarting a `Container` in a `Pod` should not be confused
with restarting a `Pod`. A `Pod` is not a process, but an environment for
running `Container`(s). A `Pod` persists until it is deleted.</mark>
...
<mark>Modifying the `Pod` template or switching to a new `Pod` template has no direct
...
manage any of the details around `Pod` templates and updates.</mark>

Kubernetes doesn't prevent us from managing `Pod`s directly, albeit, with
some limitations, as mentioned [here](https://kubernetes.io/docs/concepts/workloads/pods/#pod-update-and-replacement).
...
**Storage**: <mark>A `Pod` can specify a set of shared storage `Volume`s. All
...
survive in case one of the `Container`s within needs to be restarted.</mark>
...
the IP address and network ports. <mark>Inside a `Pod` (**and only then**), the
...
communicate with each other using standard inter-process communications (IPC)
like SystemV semaphores or POSIX shared memory. `Container`s in different
`Pod`s have distinct IP addresses and can not communicate by OS-level IPC
without special configuration. `Container`s that want to interact with a
`Container` running in a different `Pod` can use IP networking to
communicate. `Container`s within the `Pod` see the system hostname as being
the same as the configured `name` for the `Pod`.</mark>
